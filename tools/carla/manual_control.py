...
# --- Modifications (carla-semantic-scene-reconstruction) ---
# Modified from CARLA manual_control.py
# Changes:
# - added --client-fps (Pygame loop limiter)
# - added --sim-fps / --delta-seconds (simulation timing control)
# -----------------------------------
    ...
    try:
        ...
        if args.sync:
            ...
            if args.sim_fps is not None:
                settings.fixed_delta_seconds = (1.0 / args.sim_fps) if args.sim_fps > 0 else 0
        ...
        while True:
            if args.sync:
                sim_world.tick()
            clock.tick_busy_loop(args.client_fps)
...
def main():
    ...
    argparser.add_argument(
        ...
        default=20,
        type=int,
        help="limit client/Pygame loop FPS (default: 20)",
    )
    ...
    argparser.add_argument(
        ...
        default=0.05,
        type=float,
        help="set fixed delta seconds in synchronous mode (default: 0.05)",
    )
    argparser.add_argument(
        "--sim-fps",
        metavar="N",
        default=None,
        type=float,
        help="set simulation FPS in synchronous mode (overrides --delta-seconds)",
    )
