...
PROCESS_SPECS: Dict[str, ProcessSpec] = {
    ...
    "manual_control": ProcessSpec(
        ...
        command=["conda", "run", "--no-capture-output", "-n", "carla_app", "python", "-u", "-m", "tools.carla.manual_control"],
    ),
    "traffic": ProcessSpec(
        name="traffic",
        title="Traffic",
        command=["conda", "run", "--no-capture-output", "-n", "carla_app", "python", "-u", "-m", "tools.carla.generate_traffic"],
    ),
    "dataset_collect": ProcessSpec(
        name="dataset_collect",
        title="Collect Dataset",
        command=["conda", "run", "--no-capture-output", "-n", "carla_app", "python", "-u", "-m", "tools.dataset.collect_dataset"],
    ),
    "dataset_view": ProcessSpec(
        name="dataset_view",
        title="Show Dataset",
        command=["conda", "run", "--no-capture-output", "-n", "carla_app", "python", "-u", "-m", "tools.dataset.show_dataset"],
    ),
    "openpcdet_prepare_dataset": ProcessSpec(
        name="openpcdet_prepare_dataset",
        title="Prepare OpenPCDet Dataset",
        command=["conda", "run", "--no-capture-output", "-n", "openpcdet", "python", "-u", "-m", "tools.openpcdet.prepare_dataset"],
    ),
    "openpcdet_train": ProcessSpec(
        name="openpcdet_train",
        title="Train OpenPCDet",
        command=["conda", "run", "--no-capture-output", "-n", "openpcdet", "python", "-u", "-m", "tools.openpcdet.train"],
    ),
    "openpcdet_test": ProcessSpec(
        name="openpcdet_test",
        title="Test OpenPCDet",
        command=["conda", "run", "--no-capture-output", "-n", "openpcdet", "python", "-u", "-m", "tools.openpcdet.test"],
    ),
    "lanedet_prepare_dataset": ProcessSpec(
        name="lanedet_prepare_dataset",
        title="Prepare LaneDet Dataset",
        command=["conda", "run", "--no-capture-output", "-n", "lanedet", "python", "-u", "-m", "tools.lanedet.prepare_dataset"],
    ),
    "lanedet_main": ProcessSpec(
        name="lanedet_main",
        title="Run LaneDet",
        command=["conda", "run", "--no-capture-output", "-n", "lanedet", "python", "-u", "-m", "tools.lanedet.main"],
    ),
    "benchmark_openpcdet": ProcessSpec(
        name="benchmark_openpcdet",
        title="Benchmark OpenPCDet",
        command=["conda", "run", "--no-capture-output", "-n", "openpcdet", "python", "-u", "-m", "tools.benchmark.benchmark_openpcdet"],
    ),
    "benchmark_lanedet": ProcessSpec(
        name="benchmark_lanedet",
        title="Benchmark LaneDet",
        command=["conda", "run", "--no-capture-output", "-n", "lanedet", "python", "-u", "-m", "tools.benchmark.benchmark_lanedet"],
    ),
    "benchmark_view_predictions": ProcessSpec(
        name="benchmark_view_predictions",
        title="View Predictions",
        command=["conda", "run", "--no-capture-output", "-n", "carla_app", "python", "-u", "-m", "tools.benchmark.view_predictions"],
    ),
    "stream_producer": ProcessSpec(
        name="stream_producer",
        title="Streaming Producer",
        command=["conda", "run", "--no-capture-output", "-n", "carla_app", "python", "-u", "-m", "tools.streaming.live_producer"],
    ),
    "stream_aggregator": ProcessSpec(
        name="stream_aggregator",
        title="Streaming Aggregator",
        command=["conda", "run", "--no-capture-output", "-n", "carla_app", "python", "-u", "-m", "tools.streaming.live_aggregator"],
    ),
    "stream_visualizer": ProcessSpec(
        name="stream_visualizer",
        title="Streaming Visualizer",
        command=["conda", "run", "--no-capture-output", "-n", "carla_app", "python", "-u", "-m", "tools.streaming.live_visualizer"],
    ),
    "stream_openpcdet": ProcessSpec(
        name="stream_openpcdet",
        title="Streaming OpenPCDet",
        command=["conda", "run", "--no-capture-output", "-n", "openpcdet", "python", "-u", "-m", "tools.streaming.live_openpcdet_inference"],
    ),
    "stream_lanedet": ProcessSpec(
        name="stream_lanedet",
        title="Streaming LaneDet",
        command=["conda", "run", "--no-capture-output", "-n", "lanedet", "python", "-u", "-m", "tools.streaming.live_lanedet_inference"],
    ),
}
